...
model = Conv1D(32, kernel_size=3, strides=2, padding="same")(h_input)
...
test_kigo = np_utils.to_categorical(ikigo[range(1000)],vocab_size)
for iteration in range(iterations):
    idx = np.random.randint(1000, ihaiku.shape[0], batch_size)
    haiku = np.array([w2v[h] for h in ihaiku[idx]])
    kigo = np_utils.to_categorical(ikigo[idx],vocab_size)
